...
function playRussian(){if(russianBusy)return;if(!russianChoice)return toast("Choose a chamber first.");const wager=Number($("#russian-wager").value);if(!canPay(wager))return;russianBusy=true;state.credits-=wager;renderHeader();const loaded=1+Math.floor(Math.random()*6),chamber=$("#chamber");chamber.classList.add("spinning");$("#russian-message").textContent=`Chamber ${russianChoice} locks into place…`;setTimeout(()=>{const safe=russianChoice!==loaded;if(safe)russianStreak++;else russianStreak=0;const mult=1.18+Math.min(5,russianStreak-1)*.08,win=safe?Math.round(wager*mult*(1+state.upgrades.payout*.05)):0;chamber.classList.remove("spinning");chamber.classList.toggle("bang",!safe);$("#russian-streak").textContent=russianStreak;$("#russian-payout").textContent=`${(1.18+Math.min(5,russianStreak)*.08).toFixed(2)}×`;$("#russian-message").textContent=safe?`CLICK! Chamber ${russianChoice} was safe · ${russianStreak} streak · Won ${fmt(win)}!`:`BANG! The round was in chamber ${loaded}. Streak reset.`;setTimeout(()=>chamber.classList.remove("bang"),650);russianBusy=false;record("russian",wager,win);},1400);}
